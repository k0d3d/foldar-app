import { ITEM_LABEL, LANGUAGE } from "../../config/default";

export function AppLanguage() {
  const lang = LANGUAGE
  const label = ITEM_LABEL
  return Language(label)[lang]
}

export function Language(label = "item") {
  return {
    "set":"eng",
    "eng":{
      "items": {
